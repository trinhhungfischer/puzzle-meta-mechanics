...
export async function POST(req: Request) {
  try {
    ...
    if (!Array.isArray(data)) {
      return NextResponse.json({ error: 'Payload data must be an array of games.' }, { status: 400 })
    }

    let count = 0
    const report = {
      newGenres: new Set<string>(),
      newPlatforms: new Set<string>(),
      newMechanics: new Set<string>(),
      errors: [] as string[]
    }

    // Fetch existing slugs for dry run validation
    const existingGenres = new Set((await prisma.genre.findMany({ select: { slug: true } })).map(g => g.slug))
    const existingPlatforms = new Set((await prisma.platform.findMany({ select: { slug: true } })).map(p => p.slug))
    const existingMechanics = new Set((await prisma.mechanic.findMany({ select: { slug: true } })).map(m => m.slug))

    for (const [index, gameData] of data.entries()) {
      if (!gameData.title) {
        report.errors.push(`Game at index ${index} is missing a title.`)
        continue
      }
      ...
      if (Array.isArray(gameData.genres)) {
        for (const g of gameData.genres) {
          ...
          if (!existingGenres.has(gSlug)) {
            report.newGenres.add(g)
          }
          if (!isDryRun) {
      ...
      if (!isDryRun) {
        ...
        // Just dry run validations for Platforms and Mechanics
        if (Array.isArray(gameData.platforms)) {
          for (const p of gameData.platforms) {
            if (p.name) {
              const pSlug = slugify(p.name)
              if (!existingPlatforms.has(pSlug)) {
                report.newPlatforms.add(p.name)
              }
            }
          }
        }
        if (Array.isArray(gameData.mechanics)) {
          for (const m of gameData.mechanics) {
            if (m.name) {
              const mSlug = slugify(m.name)
              if (!existingMechanics.has(mSlug)) {
                report.newMechanics.add(m.name)
              }
            }
          }
        }
    ...
    return NextResponse.json({ 
      ...
      dryRun: isDryRun,
      report: {
        newGenres: Array.from(report.newGenres),
        newPlatforms: Array.from(report.newPlatforms),
        newMechanics: Array.from(report.newMechanics),
